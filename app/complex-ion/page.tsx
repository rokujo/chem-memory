...
import { ChevronLeft, Home as HomeIcon, Layers, Grid3x3, Palette, Sparkles } from 'lucide-react';
import { COMPLEXES } from './lib/data';
import { loadProgress, summarize, type ProgressSummary } from './lib/progress';
import { LigandView } from './components/LigandView';
import { OverviewList } from './components/OverviewList';
import { QuizColor } from './components/QuizColor';
import { QuizFormation } from './components/QuizFormation';
...
export default function ComplexIonPage() {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-slate-100">
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        <div className="mt-6">
          ...
          {screen.kind === 'view_ligand' && <LigandView />}
          {screen.kind === 'view_overview' && <OverviewList />}
          {screen.kind === 'quiz_color' && <QuizColor />}
          {screen.kind === 'quiz_formation' && <QuizFormation />}
        </div>
      </div>
    </div>
  );
...
function Home({ setScreen }: { setScreen: (s: Screen) => void }) {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <h2 className="text-lg font-bold">錯イオンの組成式と色</h2>
        <p className="text-sm text-slate-300 mt-1 leading-relaxed">
          NH₃・CN⁻・OH⁻ を配位子とする {COMPLEXES.length} 種類の錯イオン。
          中心金属と配位子の組合せで組成式・色・配位数が決まる。
        </p>
      </div>
      ...
      <section>
        ...
        <div className="space-y-2">
          <CardItem
            icon={<Layers size={20} />}
            title="配位子別ビュー"
            description="NH₃ / CN⁻ / OH⁻ のタブで分類して見る"
            onClick={() => setScreen({ kind: 'view_ligand' })}
          />
          <CardItem
            icon={<Grid3x3 size={20} />}
            title="俯瞰一覧"
            description="全錯イオンをスクロールで一気に見る"
      ...
      <section>
        ...
        <div className="space-y-2">
          ...
          <CardItem
            ...
            accent="orange"
            title="生成クイズ"
            description="Cu²⁺ + 過剰NH₃ で何ができる? を4択"
            onClick={() => setScreen({ kind: 'quiz_formation' })}
          />
        </div>
      </section>
    </div>
