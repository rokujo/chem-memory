...
const STORAGE_KEY = 'chem-memory:electrolysis:progress:v1';
...
export function summarize(progress: Progress, now: number = Date.now()): ProgressSummary {
  ...
  for (const e of ELECTROLYSIS) {
    const entry = getEntry(progress, e.id);
