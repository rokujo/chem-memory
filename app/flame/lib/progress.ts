...
const STORAGE_KEY = 'chem-memory:flame:progress:v1';
...
export function getEntry(progress: Progress, flameId: string): ProgressEntry {
  return progress[flameId] ?? emptyEntry();
}
...
export function recordAnswer(
  ...
  const prev = progress[flameId] ?? emptyEntry();
  return { ...progress, [flameId]: applyResult(prev, correct, now) };
}
...
export function summarize(progress: Progress, now: number = Date.now()): ProgressSummary {
  ...
  for (const f of FLAMES) {
    const e = getEntry(progress, f.id);
