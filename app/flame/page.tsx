...
import { ChevronLeft, Home as HomeIcon, Flame, Brain, Target } from 'lucide-react';
import { FLAMES } from './lib/data';
import { loadProgress, summarize, type ProgressSummary } from './lib/progress';
import { OverviewList } from './components/OverviewList';
import { QuizFlameColor } from './components/QuizFlameColor';
import { QuizColorFlame } from './components/QuizColorFlame';

type Screen =
  | { kind: 'home' }
  | { kind: 'view_overview' }
  | { kind: 'quiz_flame_color' }
  | { kind: 'quiz_color_flame' };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-slate-100">
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        <div className="mt-6">
          ...
          {screen.kind === 'quiz_flame_color' && <QuizFlameColor />}
          {screen.kind === 'quiz_color_flame' && <QuizColorFlame />}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      <div
        ...
        style={{ background: 'linear-gradient(135deg, #f43f5e22 0%, transparent 60%)' }}
      >
        <h2 className="text-lg font-bold">炎色反応の暗記</h2>
        <p className="text-sm text-slate-300 mt-1 leading-relaxed">
          アルカリ金属・アルカリ土類金属・銅 {FLAMES.length} 元素の炎色反応。
          花火の色と結びつけて覚える。
        </p>
      </div>
      ...
      <section>
        ...
        <div className="space-y-2">
          <CardItem
            ...
            accent="rose"
            title="元素→色クイズ"
            description="Li の炎色は? を 4択(色名+色チップ)"
            onClick={() => setScreen({ kind: 'quiz_flame_color' })}
          />
          <CardItem
            icon={<Target size={20} />}
            accent="rose"
            title="色→元素クイズ"
            description="赤紫色の炎色反応をするのは? を 4択(元素)"
            onClick={() => setScreen({ kind: 'quiz_color_flame' })}
          />
        </div>
      </section>
    </div>
    ...
    <div className="rounded-2xl p-4 bg-slate-800/30 border border-slate-700/40">
      ...
      <div className="flex h-2 rounded-full overflow-hidden bg-slate-900/60">
        ...
        {summary.learningCount > 0 && (
          <div className="bg-rose-500" style={{ width: `${(summary.learningCount / summary.total) * 100}%` }} />
        )}
        {summary.dueCount > 0 && (
      ...
      <div className="flex flex-wrap gap-x-3 gap-y-1 mt-2 text-[11px] text-slate-400">
        ...
        <ProgressLegend color="bg-rose-500" label="学習中" count={summary.learningCount} />
...
function CardItem({
  ...
  const accentClass =
    accent === 'rose'
      ...
      : 'bg-slate-500/10 text-slate-300 group-hover:bg-slate-500/20';
  return (
    <button
      onClick={onClick}
