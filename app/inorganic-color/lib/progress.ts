...
const STORAGE_KEY = 'chem-memory:inorganic-color:progress:v1';
...
export function getEntry(progress: Progress, substanceId: string): ProgressEntry {
  return progress[substanceId] ?? emptyEntry();
}
...
export function recordAnswer(
  ...
  const prev = progress[substanceId] ?? emptyEntry();
  return { ...progress, [substanceId]: applyResult(prev, correct, now) };
}

export function recordAnswers(
  progress: Progress,
  updates: Array<{ substanceId: string; correct: boolean }>,
  now: number = Date.now(),
): Progress {
  const next: Progress = { ...progress };
  for (const u of updates) {
    const prev = next[u.substanceId] ?? emptyEntry();
    next[u.substanceId] = applyResult(prev, u.correct, now);
  }
  return next;
}
...
export function summarize(progress: Progress, now: number = Date.now()): ProgressSummary {
  ...
  for (const s of SUBSTANCES) {
    const e = getEntry(progress, s.id);
