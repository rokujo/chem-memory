import { HALF_REACTIONS } from './data';
...
const STORAGE_KEY = 'chem-memory:redox:progress:v1';
...
export function getEntry(progress: Progress, halfReactionId: string): ProgressEntry {
  return progress[halfReactionId] ?? emptyEntry();
}
...
export function recordAnswer(
  progress: Progress,
  halfReactionId: string,
  correct: boolean,
  now: number = Date.now(),
): Progress {
  const prev = progress[halfReactionId] ?? emptyEntry();
  return { ...progress, [halfReactionId]: applyResult(prev, correct, now) };
}
...
export function summarize(progress: Progress, now: number = Date.now()): ProgressSummary {
  ...
  for (const r of HALF_REACTIONS) {
    const e = getEntry(progress, r.id);
